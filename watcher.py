...
def revisar(avisar_nuevos: bool = True) -> int:
    pedidos = conf.leer_pedidos(limite=100)
    reales = [p for p in pedidos if not p.get("es_prueba")]
    vistos = _seen()
    nuevos = [p for p in reales if p["pedido"] not in vistos]
    ...
    if avisar_nuevos:
        ...
        for p in sorted(nuevos, key=lambda x: x["pedido"]):
            ...
            if ok:
                vistos.add(p["pedido"])
                print(f"avisado {p['pedido']} {p['cliente']}")
            else:
